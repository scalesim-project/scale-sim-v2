...
private:
    ...
    void schedule(request_queue& q){
        ...
        issue_cmd(first_cmd, req.addr_vec.data(),req.addr,req.depart);
    ...
    void issue_cmd(typename T::Command cmd, int* addr_vec, long req_addr, long req_depart)
    {
        ...
        if (print_cmd_trace){
            if (channel->spec->command_name[int(cmd)].c_str() == "WR")
                printf("WR %lx %d %d\n", req_addr, req_depart, clk);
