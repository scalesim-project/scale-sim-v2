...
protected:
    ...
    VectorStat addr_vector;
    VectorStat latency_vector;
...
public:
    ...
    void tick()
    {
        ...
        if (pending.size()) {
            ...
            if (req.depart <= clk) {
                if (req.depart - req.arrive > 1) { // this request really accessed a row
                  ...
                  //latency_vector.append(req.depart-req.arrive);
        ...
        if (!is_valid_req) {
            ...
            if (!victim.empty()){
                issue_cmd(cmd, victim, req->addr,req->arrive);
            }
        ...
        issue_cmd(cmd, get_addr_vec(cmd, req),req->addr,req->arrive);
...
private:
    ...
    void issue_cmd(typename T::Command cmd, const vector<int>& addr_vec, long req_addr, long req_arrive)
    {
